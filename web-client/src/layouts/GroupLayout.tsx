...
import { Dot, Ellipsis, Link2, LogOut, Plus, History, Users, Earth, Lock, X, Trash2 } from "lucide-react";
...
import { useCancelOrLeaveGroup, useDeleteGroup, useJoinGroup, useQueryGroupById, useUpdateGroup, useUploadFile } from "../hooks";
...
const GroupLayout: FC<IProps> = ({ }) => {
    ...
    const [openDeleteGroupConfirm, setOpenDeleteGroupConfirm] = useState<boolean>(false);
    ...
    const handleOnGroupDelete = () => {
        deleteGroup(id || "", {
            onSuccess: () => {
                setOpenDeleteGroupConfirm(false);
            }
        });
    }
        ...
        <>
            {isLoading ? <GroupLayoutSkeleton /> :
                group ? <>
                                ...
                                {group?.joined && <Popover
                                    ...
                                    content={
                                        <ul>
                                            ...
                                            {group.admin &&
                                                <li onClick={() => {
                                                    handleOpenGroupActionChange(false);
                                                    setOpenDeleteGroupConfirm(true);
                                                }} className="px-3 py-2 font-medium text-red-600 rounded hover:bg-gray-100 flex cursor-pointer transition-all gap-x-2 text-sm items-center"><Trash2 className="text-red-500 text-sm" /> Giải tán nhóm</li>
                                            }
                                        </ul>
                                    }
                    ...
                    <Dialog open={openDeleteGroupConfirm} onOpenChange={setOpenDeleteGroupConfirm}>
                        <DialogContent>
                            <DialogHeader>
                                <DialogTitle>Bạn có chắc là muốn giải tán nhóm?</DialogTitle>
                                <DialogDescription className="py-5">
                                    Mọi thành viên trong nhóm sẽ không còn truy cập và hoạt động được trong nhóm nữa, bạn chắc chứ?
                                </DialogDescription>
                            </DialogHeader>
                            <DialogFooter>
                                <DialogClose>
                                ...
                                <button disabled={isPending} className="btn-danger" onClick={handleOnGroupDelete}>Giải tán</button>
                            </DialogFooter>
                        </DialogContent>
                    </Dialog>
                </> : <NotFound />}
